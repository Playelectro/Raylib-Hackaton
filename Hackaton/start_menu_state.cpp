...
void StartMenuState::InitState() {
	...
	CreateButton(GetScreenWidth() / 2 - buttonWidth / 2, GetScreenHeight() / 2.5, buttonWidth, buttonHeight, 40, "Simulate","buton", [](Actor* actor) {
		ContextState::getInstance()->SetState(new TestState());
		ContextState::getInstance()->InitState();
	});

	CreateButton(GetScreenWidth() / 2 - buttonWidth / 2, GetScreenHeight() / 1.5, buttonWidth, buttonHeight, 40, "Learn","buton", [](Actor* actor) {
